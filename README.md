...
仓库同时包含一个由 Agent 操作和预览的 React Todo 示例应用，方便验证“理解需求 → 修改项目 → 启动预览”的完整流程。
...
| `bash` | 执行 shell 命令 |
| `start_preview` | 启动 `app/` 静态预览服务，默认端口为 `8080` |
...
Agent 也可以通过 `start_preview` 启动 `app/` 的静态预览服务。该服务适合快速查看文件内容和原型，复杂的 TypeScript/React 构建仍建议使用 Vite。
